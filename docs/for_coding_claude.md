...
初回△・×が1件以上
  → ② 同日やり直し・1周目
  → ③（1周目で不正解があれば）同日やり直し・2周目
  → ④ 最後の振り返り画面
```

### 同日やり直しを開始しない条件

通常40問終了時に、初回判定の△・×が0件だった場合は、同日やり直しには進みません。通常完了画面へ進み、学習を終了します。

通常完了画面では、少なくとも以下が確認できる状態を想定します（文言・UIデザインは後続の実装指示で決めます）。
...
「今日のクリア」「2周目でクリア」「明日もう一度」は、同日やり直し結果を画面上で示すUI表示用の状態です。正式な学習進捗フィールドとは独立して扱います。

| フィールド | 用途 | 同日やり直しで変更 |
|---|---|---|
| `level` | 忘却曲線・定着判定用 | しない |
| `stableCorrectCount` | 定着判定用 | しない |
| `nextReviewAt` | 次回復習日（忘却曲線用） | しない |
| `sameDayFinalStatus` | UI表示・当日履歴用 | する（保存する場合） |

`sameDayFinalStatus` を保存する場合も、正式な学習進捗フィールドとは分離して管理します。
...
同日やり直し1周目で全問正解した場合、または2周目が終了した場合に、最後の振り返り画面を表示します。

通常40問で△・×だった問題を全件一覧表示します。  
○だった問題は表示しません。
